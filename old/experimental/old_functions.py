...
@timerfunc
def train2(X, Y, nn_architecture, epochs, learning_rate, method, n_batches, batch_size, cost_type, N_nn, kernel_a, alpha_init, alpha_rate, beta, gamma, verbose, var_epsilon, dispersion_factor=6):
...
@timerfunc
def train_experimental(X, Y, nn_architecture, epochs, learning_rate, method, n_batches, batch_size, cost_type, N_nn, kernel_a, alpha_init, alpha_rate, beta, gamma, verbose, var_epsilon, dispersion_factor=6):
    ...
    if kernel_a == "auto":
        ...
        for kernel_a in [0.0001, 0.0005,0.001,0.005, 0.01, 0.05, 0.1, 0.5, 1, 5, 10]:
            if(np.mean(np.einsum('ij -> i',np.exp(-kernel_a*norm))/N_nn)) < 0.5:
                break
    ...
    for i in range(epochs):
        ...
        for batch in range(n_batches):
            ...
            paramsf, var = update_cloud_derivative_free(paramsf, costs, learning_rate, N_nn, kernel_a, alpha, beta, gamma)
...
def update_nn_weights_old(cloud,gradients,N_nn, lr, kernel_a, alpha, beta, gamma):
             
    #get flattened weights, gradients, nn shape and weight names
    cloudf, gradientsf, nn_shape, weight_names = flatten_weights_gradients(cloud, gradients, N_nn)

    #compute kernels
    kernels =  [[kernel(cloudf[i],cloudf[j],kernel_a) for j in range(N_nn)]   for i in range(N_nn)]
    
    if beta > 0:
        gkernels = [[gkernel(cloudf[i],cloudf[j],kernel_a) for j in range(N_nn)]  for i in range(N_nn)]
    ...
    for nn in range(N_nn):    
        ...
        if alpha > 0:
            R = [[kernels[nn][j]*(cloudf[j] - cloudf[k]) for j in range(N_nn)] for k in range(N_nn)]
        ...
        if gamma > 0:
            S = [kernels[nn][j] * np.divide((cloudf[j]-cloud_mean),cloud_var) for j in range(N_nn)]
            S = np.mean(S,axis=0)
...
def update_nn_weights_derivative_free_old(cloud, cost, lr, N_nn, kernel_a, alpha, beta, gamma):
    ...
    for nn in range(N_nn):    
        ...
        Q = [gkernels[nn][j]*cost[j] + kernels[nn][j]*cost[j]*np.divide((cloudf[j]-cloud_mean),cloud_var) for j in range(N_nn)]
        Q = np.mean(Q,axis=0)
...
def update_nn_weights_profiled(cloud, gradients, N_nn, lr, kernel_a, alpha, beta, gamma):
    ...
    #get updated cloud and its variance
    cloudf, cloud_var = update_cloud(cloudf, gradientsf, N_nn, lr, kernel_a, alpha, beta, gamma) 
...
def update_nn_weights_derivative_free_profiled(cloud, cost, lr, N_nn, kernel_a, alpha, beta, gamma):
